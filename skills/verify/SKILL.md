...
Security input is required for every completed task so the final classification is explicit. It records `classification: security-sensitive|not-security-sensitive`, `surfaces`, `trust_boundaries`, `abuse_cases`, `controls_reviewed`, `targeted_checks`, `diff_review`, `scanner`, `dependency_vulnerability`, and `limitations`. For a sensitive task, at least one surface, trust boundary, abuse case, reviewed control, and passed targeted check are required. `diff_review.status` must be `passed` with non-empty evidence against the final diff; scanner/dependency-vulnerability status must be explicit; `not-available`, `not-configured`, and `not-applicable` require a non-empty reason, while `passed` requires non-empty evidence. If runtime produced candidate surfaces but the final decision is `not-security-sensitive`, include a non-empty `candidate_override_reason` and use `diff_review.status: not-applicable` when no security-focused diff review is required.
